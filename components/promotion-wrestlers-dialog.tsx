...
import { getWrestlersByPromotion } from "@/lib/database"
import type { Wrestler } from "@/types/wrestler"
import { useToast } from "@/hooks/use-toast"
import { Badge } from "@/components/ui/badge"

interface PromotionWrestlersDialogProps {
  open: boolean
...
export function PromotionWrestlersDialog({
  open,
  onOpenChange,
  promotionId,
  promotionName,
}: PromotionWrestlersDialogProps) {
  ...
  const loadWrestlers = async () => {
    setIsLoading(true)
    try {
      const data = await getWrestlersByPromotion(promotionId)
      setWrestlers(data)
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[80vh] overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Wrestlers in {promotionName}</DialogTitle>
          <DialogDescription>View all wrestlers associated with {promotionName}.</DialogDescription>
        </DialogHeader>
        <div className="py-4">
          {isLoading ? (
            ...
            <div className="flex h-32 items-center justify-center rounded-lg border border-dashed">
              <p className="text-muted-foreground">No wrestlers found in this promotion</p>
            </div>
          )}
        </div>
      </DialogContent>
