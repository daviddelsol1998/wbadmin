...
import { getWrestlersByFaction } from "@/lib/database"
import type { Wrestler } from "@/types/wrestler"
import { useToast } from "@/hooks/use-toast"

interface FactionWrestlersDialogProps {
  open: boolean
...
export function FactionWrestlersDialog({ open, onOpenChange, factionId, factionName }: FactionWrestlersDialogProps) {
  ...
  const loadWrestlers = async () => {
    setIsLoading(true)
    try {
      const data = await getWrestlersByFaction(factionId)
      setWrestlers(data)
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[80vh] overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Wrestlers in {factionName}</DialogTitle>
          <DialogDescription>View all wrestlers associated with {factionName}.</DialogDescription>
        </DialogHeader>
        <div className="py-4">
          {isLoading ? (
                  ...
                  <div className="flex-1">
                    ...
                    {wrestler.championships && wrestler.championships.length > 0 && (
                      <div className="mt-1 text-sm text-muted-foreground">
                        Championships: {wrestler.championships.map((c) => c.name).join(", ")}
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="flex h-32 items-center justify-center rounded-lg border border-dashed">
              <p className="text-muted-foreground">No wrestlers found in this faction</p>
            </div>
          )}
        </div>
      </DialogContent>
